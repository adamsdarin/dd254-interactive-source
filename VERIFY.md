...
- Release assets carry a **build provenance attestation** (Sigstore-signed,
  ...
  One attestation per release covers both HTML files. Add `--format json` to see
  what was verified: each subject's name and SHA-256, and the tag in
  `sourceRepositoryRef`. Check that your file's hash is one of the subjects and
  that the tag is the release you meant to download. (The plain output can be
  empty even when verification succeeds.)

  Because the demonstration build is a subject too, the live demo can be checked
  the same way: download the served page, hash it, and compare against the
  `DD254_Interactive_vNNN_DEMO.HTM` subject.

- `SHA256SUMS.txt` on the release is generated by the same workflow run as the
  assets it lists, so it confirms the download is complete and intact, not who
  built it. `rebuild_kit.tar.gz` is the split you would otherwise produce in § 1;
  its `rebuild.py` reassembles the parts into a file you can hash against the
  release.
