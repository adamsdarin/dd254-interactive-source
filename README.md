# DD‑254 Interactive — Codex Astra v2.0.1
...
| | |
|---|---|
| **Try it now** | *(demo build — fictitious data, nothing real)* → `https://adamsdarin.github.io/dd254-interactive/` — the exact v2.0.1 demo build, which opens with an example portfolio, covered by the release's signed provenance |
| **Download** | [Releases](https://github.com/adamsdarin/dd254-interactive-source/releases/latest) → `DD254_Interactive_v2.0.1.HTM` |
| **Read the code** | [`01_TOOL/`](01_TOOL/) |
| **Verify it yourself** | [VERIFY.md](VERIFY.md) |

### Running it

1. Download `DD254_Interactive_v2.0.1.HTM` from the latest release.
...
- **Item 3 on Revisions and Finals.** The Original's date stays in Item 3a and now
  prints on both PDFs; a missing or changed original date blocks export. Revisions
  are numbered 1, 2, 3 within each Original, and the number must be a whole number.
  An award Original spawned from a Solicitation starts with 3a empty.

## What changed in v2.0.0

- **Restoring a backup no longer deletes newer templates.** Restore shows what it
  would change in each library and merges: templates in both return to the backup's
  version, templates only in the backup are added, and templates created since are
  kept. Replacing every list is a separate choice that first takes a Full Backup.
- **Fixed:** Undo after importing a colleague's templates emptied the library.
...
- **Biennial review attribution corrected.** The manual credited the dashboard's
  review clock to DoDI 5220.22. The clock follows the DD Form 254 Instructions,
  Item 3b(3) (review of revisions); the GCA's own biennial review is a separate
  government duty in DoDM 5220.32 Volume 1, which the CO package now cites.

## What changed in v1.15.6

- **Final-due prompt.** A tool-only period of performance end date on the card
  (never printed) prompts at 120, 60 and 30 days before performance ends and
  before the two-year retention window closes, until a Final is issued, with a
  calendar invite. Extending it for an option year never suggests a revision.
- **Example portfolio in the demo.** The demonstration build opens with
  fictitious DD-254s in every status.

## What changed in v1.15.5

- **Countersignature badge on SAP subcontracts only.** Revisions of prime
  contracts and non-SAP subcontracts no longer show "no countersignature"; the
  subcontractor signature is a DoDM 5205.07 SAP rule.
- **SAP or non-SAP for life.** A spawned issuance whose SAP flag differs from its
  parent is a blocking error.
- **Redraft baseline.** A revision summary can compare with the Original or any
  earlier Revision, not only the issuance it was spawned from.
- **Fixed:** long confirmation dialogs ran off the screen with no reachable
  buttons; they now scroll inside the window.

## What changed in v1.15.4

- **Revision summary in Item 13.** A Revision spawned on the dashboard gets a
  plain-language summary of what changed at the top of Item 13, kept up to date
  until you edit it by hand, with Redraft and Remove.
- **Point-of-contact-only revisions** raise a warning, following the DD Form 254
  instructions.
- **Fixed:** a Revision that raised Item 1a or 1b was blocked by a subcontract
  flow-down error comparing it with its own Original.
...
[Implementation status and limitations](02_DOCS/product_planning/IMPLEMENTATION_v2.0.1.md)
...
- [Release assessment and change list](02_DOCS/RELEASE_ASSESSMENT_v2.0.1.md)
- [Implementation notes for this release](02_DOCS/product_planning/IMPLEMENTATION_v2.0.1.md)
