...
```
certutil -hashfile DD254_Interactive_v1.15.2.HTM SHA256
```
...
```
python split.py DD254_Interactive_v1.15.2.HTM parts
```
...
Its exact size for each release is in `BUILD_FACTS.md`. For v1.15.2 it is about
884 KB after the three replaceable components are carved out, which the
shipped-file accounting splits into roughly 668 KB of unobfuscated, commented
application code and 216 KB of markup and CSS. The figure changes release to
...
- **It does not replace code review.** It reduces what must be reviewed from
  about 2.2 MB to about 884 KB (v1.15.2; see `BUILD_FACTS.md`). Someone still
  has to read that.
