...
```
certutil -hashfile DD254_Interactive_v1.15.4.HTM SHA256
```
...
```
python split.py DD254_Interactive_v1.15.4.HTM parts
```
...
Its exact size for each release is in `BUILD_FACTS.md`. For v1.15.4 it is about
914 KB after the three replaceable components are carved out, which the
shipped-file accounting splits into roughly 697 KB of unobfuscated, commented
...
- **It does not replace code review.** It reduces what must be reviewed from
  about 2.2 MB to about 914 KB (v1.15.4; see `BUILD_FACTS.md`). Someone still
  has to read that.
