...
```
certutil -hashfile DD254_Interactive_v1.15.1.HTM SHA256
```
...
```
python split.py DD254_Interactive_v1.15.1.HTM parts
```
...
Its exact size for each release is in `BUILD_FACTS.md`. For v1.15.1 it is about
870 KB after the three replaceable components are carved out, which the
shipped-file accounting splits into roughly 655 KB of unobfuscated, commented
...
- **It does not replace code review.** It reduces what must be reviewed from
  about 2.2 MB to about 870 KB (v1.15.1; see `BUILD_FACTS.md`). Someone still
  has to read that.
