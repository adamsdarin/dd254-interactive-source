...
```
certutil -hashfile DD254_Interactive_v1.14.0.HTM SHA256
```
...
```
python split.py DD254_Interactive_v1.14.0.HTM parts
```
...
Its exact size for each release is in `BUILD_FACTS.md`. For v1.14.0 it is about
824 KB after the three replaceable components are carved out, which the
shipped-file accounting splits into roughly 614 KB of unobfuscated, commented
application code and 210 KB of markup and CSS. The figure changes release to
release — v1.14.0 removed two features — so read the one for your file.
...
- **It does not replace code review.** It reduces what must be reviewed from
  about 2.1 MB to about 824 KB (v1.14.0; see `BUILD_FACTS.md`). Someone still
  has to read that.
