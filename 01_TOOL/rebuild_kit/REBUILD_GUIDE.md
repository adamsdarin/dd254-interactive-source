...
```
certutil -hashfile DD254_Interactive_v1.14.1.HTM SHA256
```
...
```
python split.py DD254_Interactive_v1.14.1.HTM parts
```
...
Its exact size for each release is in `BUILD_FACTS.md`. For v1.14.1 it is about
856 KB after the three replaceable components are carved out, which the
shipped-file accounting splits into roughly 641 KB of unobfuscated, commented
application code and 215 KB of markup and CSS. The figure changes release to
release — v1.14.0 removed two features and v1.14.1 added corrections and their
...
- **It does not replace code review.** It reduces what must be reviewed from
  about 2.2 MB to about 856 KB (v1.14.1; see `BUILD_FACTS.md`). Someone still
  has to read that.
- **It provides no chain of custody for the application code.** That part was
  ...
  majority of it was generated by a large language model under that person's
  direction. Its public version control history begins on 26 August 2026 with
  the v190 release; earlier development is not recorded, and most commits are
  unsigned. Releases from v1.10.0 carry signed build provenance, which proves
  which workflow built a file from which commit — not who wrote the code or how
  it was reviewed.
